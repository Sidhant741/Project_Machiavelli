...
from .gradio_ui import GRADIO_JS_CODE, build_gradio_app, get_gradio_display_title
...
class WebInterfaceManager:
    ...
    async def reset_environment(self, **kwargs) -> Dict[str, Any]:
        ...
        self.episode_state.episode_id = getattr(state, "episode_id", "default")
    ...
    async def step_environment(self, action_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        action_log = ActionLog(
            ...
            action=action.model_dump(exclude={"metadata"}) if hasattr(action, "model_dump") else action,
        ...
        # Update episode state
        self.episode_state.episode_id = getattr(state, "episode_id", "default")
        self.episode_state.step_count = getattr(state, "step_count", 0)
        self.episode_state.current_observation = serialized["observation"]
...
def create_web_interface_app(
    ...
    # Normalize path to avoid double slashes; preserve "/" as root for mount
    path = path.rstrip("/") or "/"
    # route_prefix is empty at root so f"{route_prefix}/foo" => "/foo" (no double slash)
    route_prefix = "" if path == "/" else path

    # Web API routes first (so they take precedence over Gradio mount)
    @app.get(f"{route_prefix}/metadata")
    ...
    # When Gradio is mounted at root it catches everything — register legacy
    # /web redirects BEFORE the mount so Starlette sees them first.
    if path == "/":
        from fastapi.responses import RedirectResponse as _RR

        @app.get("/web")
        @app.get("/web/")
        def _redirect_web_to_root():
            """Redirect old /web path to root for backward compatibility."""
            return _RR(url="/", status_code=301)

    # Mount tasks/images/ as static files so the browser can load task images.
    # web_interface.py lives at: <repo>/src/openenv/core/env_server/
    # tasks/images lives at:     <repo>/tasks/images/
    # → 4 levels up from this file
    _here = os.path.dirname(os.path.abspath(__file__))
    _tasks_images_dir = os.path.normpath(
        os.path.join(_here, "..", "..", "..", "..", "tasks", "images")
    )
    # Also try relative to cwd (works when running from repo root)
    _cwd_tasks_images = os.path.join(os.getcwd(), "tasks", "images")
    if os.path.isdir(_tasks_images_dir):
        app.mount("/tasks/images", StaticFiles(directory=_tasks_images_dir), name="task_images")
    elif os.path.isdir(_cwd_tasks_images):
        app.mount("/tasks/images", StaticFiles(directory=_cwd_tasks_images), name="task_images")
